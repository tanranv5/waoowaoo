...
import { QWEN_VOICE_DESIGN_PATH, resolveQwenConfiguredUrl } from '@/lib/qwen-api'
...
export async function createVoiceDesign(input: VoiceDesignInput, apiKey: string): Promise<VoiceDesignResult> {
    ...
    try {
        const response = await fetch(resolveQwenConfiguredUrl('QWEN_VOICE_DESIGN_BASE_URL', QWEN_VOICE_DESIGN_PATH), {
            method: 'POST',
            headers: {
